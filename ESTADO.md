...
- `main.rs` maneja `Vec<Particle>` con dos partículas: cuadrícula de campo total calculada y almacenada en `Vec<ElectricFieldPoint>` una vez por fotograma, extremos reales de magnitud (excluyendo `0.0`) para la escala de color, y dibujo de cada partícula y cada segmento a partir de esos datos ya calculados.
...
- `cargo fmt --check`, `cargo check` y `cargo test` (18 pruebas) terminan correctamente.

## Siguiente paso

Reanudar la comprensión y devolución gradual de las construcciones a formas idiomáticas cuando se entienda cada una, o avanzar hacia la siguiente funcionalidad física/interactiva.
